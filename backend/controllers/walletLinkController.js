...
const LinkedBank = require('../models/LinkedBank'); // Giữ để dùng trong checkLinked nếu cần, nhưng ưu tiên service

// Request liên kết (gọi request OTP từ ngân hàng)
const requestLink = async (req, res) => {
  try {
    const { accountNumber } = req.body;
    const userId = req.user.id; // walletId và userId giống nhau
    if (!accountNumber) {
...
// Check nếu đã liên kết (dựa trên ACTIVE)
