...
const requestDepositOTP = async (userId, bankAccountNumber, amount) => {
  ...
  console.group('🏦 BANK API ERROR');

  // 1️⃣ Lỗi tổng quát
  console.log('Message:', error.message);
  console.log('Error name:', error.name);

  // 2️⃣ Axios config (request đã gửi)
  if (error.config) {
  ...
  // 3️⃣ Response từ ngân hàng (nếu có)
  if (error.response) {
    console.log('⬅️ RESPONSE FROM BANK');
  ...
  // 4️⃣ Không có response (timeout, ECONNREFUSED, CORS…)
  if (!error.response) {
    console.log('❌ NO RESPONSE RECEIVED');
    console.log('Raw error:', error.toJSON?.() || error);
  }

  console.groupEnd();

  // 5️⃣ Trả message gọn cho FE
  const msg =
    error.response?.data?.message ||
    error.message ||
    'Không thể kết nối đến ngân hàng';
...
const verifyDepositOTP = async (userId, transactionId, otp) => {
  ...
  console.error({
    ...
    source: error.response ? 'BANK_API / OTP' : 'INTERNAL'
  });
  ...
  throw new Error(
    error.response?.data?.message || 'OTP không hợp lệ hoặc giao dịch thất bại'
  );
}
