...
function VerifyOTP() {
  ...
  const handleVerify = async (e) => {
    ...
    try {
      ...
      const userIdFromResponse = res.data.userId || res.data.id;
      const userIdFromToken = getUserIdFromToken(token);
      const finalUserId = userIdFromResponse || userIdFromToken;
      ...
      if (finalUserId) {
        ...
        console.log('✅ UserId saved:', finalUserId);
      } else {
        console.warn('⚠️ No userId found in response or token');
      }

      alert('Đăng nhập thành công!');
      
      window.location.href = res.data.role === 'admin' ? '/choose' : '/welcome';
  ...
  const handleResend = async () => {
    setLoading(true);
    try {
      await axios.post(`${API_BASE_URL}/api/resend-otp`, { tempUserId });
